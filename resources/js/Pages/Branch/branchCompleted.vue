...
const title = "Branch Completed";

// Sample table data
const tableData = ref([
  { id: 1, branch: "101", status: "Completed", service: "Repair", payment: "Gcash" },
  { id: 2, branch: "203", status: "Completed", service: "Customization", payment: "Cash" },
  { id: 3, branch: "305", status: "Completed", service: "Cleaning", payment: "Cash" },
]);
...
<div v-if="modalOpen" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
  <div class="bg-white dark:bg-gray-800 rounded-lg w-1/3 p-6">
    ...
    <div>
      ...
      <div class="mb-4">
        <strong class="text-gray-600 dark:text-gray-300">Service Warranty:</strong>
        <p class="text-gray-800 dark:text-gray-200">{{ selectedShoeDetails.warranty }}</p>
      </div>
    </div>
  </div>
</div>
