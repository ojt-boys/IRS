...
// Add a function for "Received" action (outside the table)
const markBatchAsReceived = () => {
  if (selectedBatch.value !== null) {
    // Example: Change status or perform action for the selected batch
    console.log(`Batch ${selectedBatch.value} marked as received.`);
    // Optionally update status or other data
    status.value = 'Received'; // Update status globally
  } else {
      ...
      <div class="flex flex-col flex-1 w-full">
        ...
        <main class="flex-1 p-6 w-full">
          <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            ...
            <!-- "Received" button and status beside each other -->
            <div v-if="selectedBatch !== null" class="mb-4 flex items-center space-x-4">
              <Button @click="markBatchAsReceived" class="bg-green-500 text-white">
                Mark as Received
              </Button>

              <!-- Status beside the button -->
              <span class="text-sm text-gray-600 dark:text-gray-400 bg-yellow-200 dark:bg-yellow-600 p-2 rounded-lg">
                Status: {{ status }}
              </span>
            </div>
